...
imagen_original = cv2.imread('tu_imagen.jpg')

# Convierte la imagen a formato HSV
imagen_hsv = cv2.cvtColor(imagen_original, cv2.COLOR_BGR2HSV)

# Define los rangos de colores que deseas separar en cada capa
...
rangos_colores = [
    ...
    (np.array([0, 50, 50]), np.array([10, 255, 255])),  # Rojo (segundo tono)
...
for i, rango_color in enumerate(rangos_colores):
    ...
    mascara = cv2.inRange(imagen_hsv, rango_color[0], rango_color[1])

    # Aplica la máscara a la imagen original para obtener la capa
    capa = cv2.bitwise_and(imagen_original, imagen_original, mask=mascara)

    # Agrega la capa a la lista
    capas.append(capa)

    # Guarda cada capa como una imagen separada
    cv2.imwrite(f'capa{i + 1}.png', capa)
